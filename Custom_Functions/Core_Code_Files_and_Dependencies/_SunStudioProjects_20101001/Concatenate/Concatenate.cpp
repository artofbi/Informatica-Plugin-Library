...
//static IUNICHAR *Concatenate_STR = (IUNICHAR *)L"Concatenate";
                                                ...
                                                INFA_EXPR_VALIDATE_METHODS* functions)
{
    ...
    functions->validateFunction = validateFunctionConcatenate;
    functions->getFunctionDescription = getDescriptionConcatenate;
    functions->getFunctionPrototype = getPrototypeConcatenate;
...
{    
    ...
    functions->module_init = moduleInitConcatenate;
    functions->module_deinit = moduleDeinitConcatenate;
...
{    
    functions->function_init = functionInitConcatenate;
    functions->function_deinit = functionDeinitConcatenate;
...
{    
    functions->fnInstance_init = functionInstInitConcatenate;
    functions->fnInstance_processRow = processRowConcatenate;
    functions->fnInstance_deinit = functionInstDeinitConcatenate;
...
extern "C" SAMPLE_EXPR_SPEC 
    IUNICHAR * getDescriptionConcatenate(IUNICHAR* ns, IUNICHAR* sFuncName)
{
    static IUNICHAR *uniDesc = (IUNICHAR *)L"Returns a concatenated string after assessing all input arguments. Max 50 Arguments. Min 1 Argument.";
    return uniDesc;
}

// method to get prototype of function
extern "C" SAMPLE_EXPR_SPEC 
    IUNICHAR * getPrototypeConcatenate(IUNICHAR* ns, IUNICHAR* sFuncName)
{
    static IUNICHAR *uniProt = (IUNICHAR *)L"Concatenate(Arg1 [, Arg2, Arg3, Arg4, Arg50])";
...
extern "C" SAMPLE_EXPR_SPEC 
    INFA_EXPR_STATUS validateFunctionConcatenate(IUNICHAR* ns, IUNICHAR* sFuncName, 
                                        IUINT32 numArgs, 
                                        ...
                                        INFA_EXPR_OPD_METADATA* retValue)
{
    ...
    {
        IUNICHAR *errMsg = (IUNICHAR *)L"Concatenate function takes 1 to 50 arguments";
...
extern "C" SAMPLE_EXPR_SPEC 
    INFA_EXPR_ROWSTATUS processRowConcatenate(INFA_EXPR_FUNCTION_INSTANCE_HANDLE *fnInstance, IUNICHAR **errMsg)
{
    INFA_EXPR_OPD_RUNTIME_HANDLE* retHandle = fnInstance->retHandle;

	// set initial valuses and defaults
	char strCRCHold[4001];
	//char strCRC[101];
	char strTmp[50];
	int ival;
	double fval;
	...
	////Init_CRC32_Table();
	// attempt to do it at function Init
	...
	INFA_EXPR_SetString(retHandle, strCRCHold);
	INFA_EXPR_SetLength(retHandle, strlen(strCRCHold));
...
extern "C" SAMPLE_EXPR_SPEC 
    INFA_EXPR_STATUS moduleInitConcatenate(INFA_EXPR_MODULE_HANDLE *modHandle)
{
    INFA_EXPR_STATUS exprStatus;

    // initialize the Concatenate_STR
    exprStatus.status = ISUCCESS;
    return exprStatus;
}
...
extern "C" SAMPLE_EXPR_SPEC 
    INFA_EXPR_STATUS moduleDeinitConcatenate(INFA_EXPR_MODULE_HANDLE *modHandle)
{
    INFA_EXPR_STATUS exprStatus;
    exprStatus.status = ISUCCESS;
    return exprStatus;
...
extern "C" SAMPLE_EXPR_SPEC 
    INFA_EXPR_STATUS functionInitConcatenate(INFA_EXPR_FUNCTION_HANDLE *funHandle)
{
    INFA_EXPR_STATUS exprStatus;
    exprStatus.status = ISUCCESS;
    return exprStatus;
...
extern "C" SAMPLE_EXPR_SPEC 
    INFA_EXPR_STATUS functionDeinitConcatenate(INFA_EXPR_FUNCTION_HANDLE *funHandle)
{
    INFA_EXPR_STATUS exprStatus;
    exprStatus.status = ISUCCESS;
    return exprStatus;
...
extern "C" SAMPLE_EXPR_SPEC 
    INFA_EXPR_STATUS functionInstInitConcatenate(INFA_EXPR_FUNCTION_INSTANCE_HANDLE *funInstHandle)
{
    //INFA_EXPR_STATUS exprStatus;
    //exprStatus.status = ISUCCESS;
